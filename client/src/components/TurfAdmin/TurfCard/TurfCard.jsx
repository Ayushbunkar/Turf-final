...
const TurfCard = ({
  ...
  getDirections,
  setSelectedTurf,
  distance, // <-- use this prop
  index,
  image // <-- add image prop
}) => {
  const WeatherIcon = weatherIcons[turf?.weather?.condition] || weatherIcons.sunny;

  // Use the passed image prop if available, else fallback to cardImages
  const cardImage = image || cardImages[index % cardImages.length] || "/placeholder.svg";
    ...
    >
      ...
      <div className={`relative ${isListView ? "w-64 flex-shrink-0" : ""}`}>
        <div className="relative overflow-hidden">
          <img
            src={typeof cardImage === "string" ? cardImage : "/placeholder.svg"}
            alt={turf.name || "Turf"}
