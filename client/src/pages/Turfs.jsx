...
const Turfs = () => {
  ...
  if (!isLoaded || !userPos) return <div className="text-center py-20">Fetching your location...</div>;
              ...
              <ul className="space-y-4">
                {filteredTurfs.map(t => (
                  <motion.li
                    ...
                    onClick={() => navigate(`/turf/${t.id}`)}
                    className="p-4 bg-white rounded-lg shadow cursor-pointer transition hover:border-green-600 border"
                  >
                    <h3 className="text-xl font-semibold">{t.name}</h3>
                    <p className="text-gray-600">
                      Distance: ~
                      {(Math.hypot(t.lat - userPos.lat, t.lng - userPos.lng) * 111).toFixed(1)} km
                    </p>
                  </motion.li>
                ))}
              </ul>
            ) : (
              <p className="text-center text-gray-600">No turfs found within {radiusKm} km.</p>
            )}
          </motion.div>
        </div>
      </section>
